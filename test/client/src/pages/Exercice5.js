...
import pariteContract from "../contracts/pariteContract";
...
export default function Exercice5() {
  ...
  const verifierParite = async () => {
    try {
      const entier = parseInt(nombre);
      const res = await pariteContract.methods.estPair(entier).call();
      setResult(`Le nombre ${entier} est ${res ? "pair" : "impair"}`);
    } catch (err) {
      console.error("Erreur:", err);
      setResult("Erreur lors de l'appel à estPair()");
    }
  };
